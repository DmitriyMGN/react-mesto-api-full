...
userRoutes.patch(
  ...
  celebrate({
    body: Joi.object().keys({
      avatar: Joi.string().regex(/^http(s)?:\/\/(www\.)?[a-zA-Z0-9-._~:/?#[\]@!$&'()*+,;=]{1,256}\.[a-z]{1,6}\b[a-zA-Z0-9-._~:/?#[\]@!$&'()*+,;=]*$/),
    }),
  }),
  updateUserAvatarById,
);
...
cardRoutes.post(
  ...
  celebrate({
    body: Joi.object().keys({
      ...
      link: Joi.string().required().regex(/^http(s)?:\/\/(www\.)?[a-zA-Z0-9-._~:/?#[\]@!$&'()*+,;=]{1,256}\.[a-z]{1,6}\b[a-zA-Z0-9-._~:/?#[\]@!$&'()*+,;=]*$/),
    }),
  }),
  createCard,
);
